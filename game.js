...
class Columns{
	...
	switch(){
		if (this._next != null && this.game._cell_list.length != 0){
			var next = this._next
	...
	mode(){
		...
		$(".menu-item,#scoreboard,html").css('color',this.text)
		// $("html").css('color',this.text)
	}
	display(){
		let x=0
		let y=0
...
function update(time=0){
	...
	lastTime = time;
	if(dropCounter>dropInterval && test.game._running){
		test.game.clear();
		test.game.drop();
		test.game.refresh()
		dropCounter = 0;
